...
class GDCVideos < Test::Unit::TestCase
  def setup
    @verification_errors = []
  ...
  def test_events
	...
	# OPEN PAGE
    @selenium.open "http://www.ign.com/events/gdc/2011/videos"
	
	# BLOGROLL TITLE
	# Blogroll title contains "Videos"
	begin
		assert /^[\s\S]*Videos[\s\S]*$/ =~ @selenium.get_text("css=h1[class=\"contentHeader typekit\"]"), "Videos blogroll tab -- blogroll title is wrong"
	...
	for j in 1..8 do #Iterate through each blogroll nav item
		if @selenium.is_element_present("//div[@class='listElmnt-blogItem']")
			...
			indexofpic = 0
			puts "-----------------------"
			puts "There are no blogroll entries in the "+j.to_s+" blogroll nav of the videos page"
			puts "-----------------------"
		end
		if @selenium.is_element_present("css=ul.ign-blogrollFilters")
			...
			begin
				assert @selenium.is_element_present("css=ul.ign-blogrollFilters"), "The blogroll nav is not visible when the blogroll is sorted by the "+j.to_s+" list item on the Videos page, and script could not continue"
			rescue Test::Unit::AssertionFailedError
				@verification_errors << $!
			end
			break
		...
		if @selenium.is_element_present("css=div.pagination")
			...
			if entries > 9
				puts "--------------------------"
				puts "--------------------------"
				puts "==> The "+current_tab+" tab on the Video page has 10 entries but no pagination"
				puts "--------------------------"
				puts "--------------------------"
			end
		end
