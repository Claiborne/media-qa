...
describe "cheats" do
  ...
  it "should return cheats for xbox 360" do
   response = RestClient.get "http://#{@config.options['baseurl']}/v1/cheats?platform=Xbox 360"     
   response.code.should eql(200)
   data = JSON.parse(response.body)
  end 

  it "should return error for unsupported platform" do
   response = RestClient.get "http://#{@config.options['baseurl']}/v1/cheats?platform=unsupported"
   response.code.should eql(200)
   data = JSON.parse(response.body)
  end
