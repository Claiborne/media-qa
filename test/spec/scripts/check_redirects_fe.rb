...
describe 'Test Redirects Frontend' do

  it 'should redirect' do
    list.each_with_index do |l, index|
      if index.even?
        RestClient.get(l){ |response, request, result, &block|
          begin
            response.code.should == 301
            response.headers[:location].should == list[index+1]
          rescue
            puts ''
            puts l
            puts response.code
            puts response.headers[:location] if response.headers[:location]
          end
        }
      end
    end
