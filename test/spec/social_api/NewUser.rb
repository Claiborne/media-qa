...
@email = "socialuser5_#{Random.rand(100-9999)}@ign.com"
...
response = RestClient.post "http://#{@config.options['baseurl']}/v1.0/social/rest/mediaItems/#{person_id[0].to_s}/@self?st=#{person_id[0].to_s}:#{person_id[0].to_s}:0:ign.com:my.ign.com:0:0",jdata, {:content_type => 'application/json'}
...
#puts $mediaItem_activity_id
  ...
  it "should return valid response code for mediaItem endpoint all" do
   response = RestClient.get "http://#{@config.options['baseurl']}/v1.0/social/rest/mediaItems/#{person_id[0].to_s}/@self/@all"
   response.code.should eql(200)
   data = JSON.parse(response.body)
  ...
  it "should return valid response code for mediaItem wishlist" do
     response = RestClient.get "http://#{@config.options['baseurl']}/v1.0/social/rest/mediaItems/#{person_id[0].to_s}/@self/@wishlist"
     response.code.should eql(200)
     data = JSON.parse(response.body)
  end
  
  it "should return valid response code for people collection" do
       response = RestClient.get "http://#{@config.options['baseurl']}/v1.0/social/rest/mediaItems/#{person_id[0].to_s}/@self/@collection"
       response.code.should eql(200)
       data = JSON.parse(response.body)
  end
...
it "should update the mediaItem" do
 ...
 response = RestClient.put "http://#{@config.options['baseurl']}/v1.0/social/rest/mediaItems/#{person_id[0].to_s}/@self?st=#{person_id[0].to_s}:#{person_id[0].to_s}:0:ign.com:my.ign.com:0:0",jdata, {:content_type => 'application/json'}
 mediaItem_activity_id= JSON.parse(response.body)["entry"]
end
...
it "should unfollow the mediaitem" do
   response = RestClient.delete "http://#{@config.options['baseurl']}/v1.0/social/rest/mediaItems/#{person_id[0].to_s}/@self/#{mediaItem_id}?st=#{person_id[0].to_s}:#{person_id[0].to_s}:0:ign.com:my.ign.com:0:0"
   data = JSON.parse(response.body)
  puts response.body
end
