...
describe "V3 Wiki API -- Create A Wiki", :stg => true do
  ...
  before(:all) do
    ...
    @url = "http://#{@config.stg['baseurl']}/wikis?oauth_token=#{TopazToken.return_token}"
    begin
...
describe "V3 Wiki API -- Get Wiki Just Created", :stg => true do
  ...
  before(:all) do
    PathConfig.config_path = File.dirname(__FILE__) + "/../../../config/v3_wiki.yml"
    @config = PathConfig.new
    @url = "http://#{@config.stg['baseurl']}/wikis/#{WikiHelper::Vars.get_id}"
    begin
...
describe "V3 Wiki API -- Update A Wiki", :stg => true do
  ...
  before(:all) do
    PathConfig.config_path = File.dirname(__FILE__) + "/../../../config/v3_wiki.yml"
    @config = PathConfig.new
    @url = "http://#{@config.stg['baseurl']}/wikis/#{WikiHelper::Vars.get_id}?oauth_token=#{TopazToken.return_token}"
    begin
  ...
  it 'should return the updated values' do
    url = "http://#{@config.stg['baseurl']}/wikis/#{WikiHelper::Vars.get_id}?fresh=true"
