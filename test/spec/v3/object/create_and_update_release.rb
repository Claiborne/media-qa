...
describe "V3 Object API -- Create Draft Release", :stg => true do
  ...
  before(:all) do
    ...
    @url = "http://#{@config.stg['baseurl']}/releases?oauth_token=#{UpdateReleaseHelperVars.return_token}&fresh=true"
    begin
...
describe "V3 Object API -- Check Draft Release", :stg => true do
  ...
  before(:all) do
    ...
    PathConfig.config_path = File.dirname(__FILE__) + "/../../../config/v3_object.yml"
    @config = PathConfig.new
    @url = "http://#{@config.stg['baseurl']}/releases/#{UpdateReleaseHelperVars.return_release_id}?oauth_token=#{UpdateReleaseHelperVars.return_token}&fresh=true"
    begin
...
describe "V3 Object API -- Check Updated Published Release", :stg => true do
  ...
  before(:all) do
    ...
    PathConfig.config_path = File.dirname(__FILE__) + "/../../../config/v3_object.yml"
    @config = PathConfig.new
    @url = "http://#{@config.stg['baseurl']}/releases/#{UpdateReleaseHelperVars.return_release_id}?fresh=true"
    begin
      @response = RestClient.get @url
    rescue => e
      raise Exception.new(e.message+" "+@url)
...
describe "V3 Object API -- Update Published", :stg => true do
  ...
  before(:all) do
    ...
    @response = RestClient.put @url, update_release_published(
        ...
        JSON.parse(RestClient.get("http://#{@config.stg['baseurl']}/companies/slug/bioware?fresh=true").body)['companyId'].to_s,
        JSON.parse(RestClient.get("http://#{@config.stg['baseurl']}/companies/slug/bioware?fresh=true").body)['companyId'].to_s,
        JSON.parse(RestClient.get("http://#{@config.stg['baseurl']}/features/slug/1080i?fresh=true").body)['featureId'].to_s,
        JSON.parse(RestClient.get("http://#{@config.stg['baseurl']}/features/slug/720p?fresh=true").body)['featureId'].to_s,
        JSON.parse(RestClient.get("http://#{@config.stg['baseurl']}/genres/slug/action?fresh=true").body)['genreId'].to_s,
        JSON.parse(RestClient.get("http://#{@config.stg['baseurl']}/genres/slug/adventure?fresh=true").body)['genreId'].to_s,
        JSON.parse(RestClient.get("http://#{@config.stg['baseurl']}/hardware/slug/xbox-360?fresh=true").body)['hardwareId'].to_s,
...
describe "V3 Object API -- Check Updated Published", :stg => true do
  ...
  it "should return the same metadata.book.metadata.slug & order values the book returns" do
    begin
      response = RestClient.get "http://#{@config.stg['baseurl']}/books/#{@data['metadata']['book']['bookId']}?fresh=true"
    rescue => e
      raise Exception.new(e.message+"http://#{@config.stg['baseurl']}/books/#{@data['metadata']['book']['bookId']}?fresh=true")
    end
  ...
  it "should return the same metadata.book.metadata.volume.metadata values the book volume returns" do
    begin
      response = RestClient.get "http://#{@config.stg['baseurl']}/volumes/#{@data['metadata']['book']['metadata']['volume']['volumeId']}?fresh=true"
    rescue => e
      raise Exception.new(e.message+"http://#{@config.stg['baseurl']}/volumes/#{@data['metadata']['book']['metadata']['volume']['volumeId']}?fresh=true")
    end
  ...
  %w(publishers developers distributors producers effects).each do |company|
    it "should return the same companies.#{company}.metadata. name, description, slug, & legacyId value the game returns" do
      begin
        response = RestClient.get "http://#{@config.stg['baseurl']}/companies/#{@data['companies'][company][0]['companyId']}?fresh=true"
      rescue => e
        raise Exception.new(e.message+"http://#{@config.stg['baseurl']}/companies/#{@data['companies'][company][0]['companyId']}?fresh=true")
      end
  ...
  it "should return the same content.metadata name, slug & legacyId values as the feature returns" do
    @data['content']['supports'].each do |content_supports|
      begin
        response = RestClient.get "http://#{@config.stg['baseurl']}/features/#{content_supports['featureId']}?fresh=true"
      rescue => e
        raise Exception.new(e.message+"http://#{@config.stg['baseurl']}/features/#{content_supports['featureId']}?fresh=true")
      end
  ...
  %w(primaryGenre secondaryGenre).each do |genre|
    ...
    it "should return the same content.#{genre} name, slug & legacyId values as the genre returns" do
      begin
        response = RestClient.get "http://#{@config.stg['baseurl']}/genres/#{@data['content'][genre]['genreId']}?fresh=true"
      rescue => e
        raise Exception.new(e.message+"http://#{@config.stg['baseurl']}/genres/#{@data['content'][genre]['genreId']}?fresh=true")
      end
  ...
  it "should return the same content.additionalGenres.name, slug & legacyId values as the genre returns for each additional genre" do
    ...
    @data['content']['additionalGenres'].each do |genre|
      ...
      begin
        response = RestClient.get "http://#{@config.stg['baseurl']}/genres/#{genre['genreId']}?fresh=true"
      rescue => e
        raise Exception.new(e.message+"http://#{@config.stg['baseurl']}/genres/#{genre['genreId']}?fresh=true")
      end
      genre_data = JSON.parse(response.body)
  ...
  it "should return the same hardware.metadata name, description, slug, shortName, legacyId, type & releaseData.date values as the hardware returns" do
    begin
      response = RestClient.get "http://#{@config.stg['baseurl']}/hardware/#{@data['hardware']['platform']['hardwareId']}?fresh=true"
    rescue => e
      raise Exception.new(e.message+"http://#{@config.stg['baseurl']}/hardware/#{@data['hardware']['platform']['hardwareId']}?fresh=true")
    end
  ...
  before(:all) do
    ...
    begin
      @response = RestClient.put @url, update_objects(
          JSON.parse(RestClient.get("http://#{@config.stg['baseurl']}/games/slug/mass-effect-2?fresh=true").body)['gameId'].to_s,
          JSON.parse(RestClient.get("http://#{@config.stg['baseurl']}/companies/slug/scea?fresh=true").body)['companyId'].to_s,
          JSON.parse(RestClient.get("http://#{@config.stg['baseurl']}/features/slug/dualshock?fresh=true").body)['featureId'].to_s,
          JSON.parse(RestClient.get("http://#{@config.stg['baseurl']}/features/slug/kinect?fresh=true").body)['featureId'].to_s,
          JSON.parse(RestClient.get("http://#{@config.stg['baseurl']}/genres/slug/fighting?fresh=true").body)['genreId'].to_s,
          JSON.parse(RestClient.get("http://#{@config.stg['baseurl']}/genres/slug/platformer?fresh=true").body)['genreId'].to_s,
          JSON.parse(RestClient.get("http://#{@config.stg['baseurl']}/hardware/slug/ps3?fresh=true").body)['hardwareId'].to_s,
          JSON.parse(RestClient.get("http://#{@config.stg['baseurl']}/movies/slug/the-matrix?fresh=true").body)['movieId'].to_s),  :content_type => "application/json"
    rescue => e
      raise Exception.new(e.message+" "+@url)
    end
...
describe "V3 Object API -- Clean up / Delete", :stg => true do
  ...
  it "should return a 404 when deleting the release" do
    expect {RestClient.get "http://#{@config.stg['baseurl']}/releases/#{UpdateReleaseHelperVars.return_release_id}?fresh=true"}.to raise_error(RestClient::ResourceNotFound)
  end
